...
def multirun(args):
    ...
    for exp in arg_dict["exp_names"]:
        ...
        train_command = ["python", "-m", "retrieve.retriever.bm25ce.train.train_bm25ce", "--dataset_name", f"{arg_dict['dataset_name']}", "--train_num", f"{arg_dict['train_num']}", "--weak_num", f"{arg_dict['weak_num']}", "--exp_name", exp]
        # for test
        print(" ".join(train_command))
        print(f"GPU {arg_dict['gpu_id']} Training: {arg_dict['dataset_name']} on {exp}")
        subprocess.call(train_command)
        eval_command = ["python", "-m", "retrieve.retriever.bm25ce.eval.evaluate_bm25ce", "--dataset_name", f"{arg_dict['dataset_name']}", "--train_num", f"{arg_dict['train_num']}", "--exp_name", exp, "--dpr_v", arg_dict["version"]]
        # for test

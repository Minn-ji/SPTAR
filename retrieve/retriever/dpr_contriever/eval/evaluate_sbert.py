...
import argparse
import json
from preprocess_dataset.data_process import load_dl, merge_queries, extract_results



data_dir = os.path.join("retrieve", "datasets")
raw_dir = os.path.join(data_dir, "raw")
beir_dir = os.path.join(raw_dir, "beir__")
...
handler = logging.FileHandler(os.path.join(model_save_path, "test_log.txt"))
...
if args.dataset_name == "msmarco":
    corpus, queries, qrels = GenericDataLoader(os.path.join(beir_dir, args.dataset_name)).load(split="dev")
    queries_19, qrels_19, qrels_binary_19 = load_dl(os.path.join(beir_dir, "TREC_DL_2019"))
    queries_20, qrels_20, qrels_binary_20 = load_dl(os.path.join(beir_dir, "TREC_DL_2020"))
else:
    corpus, queries, qrels = GenericDataLoader(os.path.join(beir_dir, args.dataset_name)).load(split="test")
...
for dataset_name in tobe_eval.keys():
    ...
    with open(os.path.join(model_save_path, f"{dataset_name}.json"), "w") as f:
